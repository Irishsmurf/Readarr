...
* **Security update to ImageSharp** — `SixLabors.ImageSharp` 3.1.7 → 3.1.12 ([GHSA-rxmq-m78w-7wmc](https://github.com/advisories/GHSA-rxmq-m78w-7wmc)). ImageSharp decodes cover art fetched from third-party metadata mirrors, so it handles untrusted input. `MailKit` 4.8.0 → 4.17.0 ([GHSA-9j88-vvj5-vhgr](https://github.com/advisories/GHSA-9j88-vvj5-vhgr)), which was unfixable while the project targeted .NET 6 and became possible once it did not ([#9](https://github.com/Irishsmurf/Readarr/issues/9)). The NuGet audit no longer blanket-ignores its own findings: advisories fail the build again, with the handful that are known and currently unfixable suppressed individually and by name.
